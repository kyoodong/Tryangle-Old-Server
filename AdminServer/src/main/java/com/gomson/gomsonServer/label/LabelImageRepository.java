...
public class LabelImageRepository {
    ...
    public Boolean scoreImage(String imageId, int score, String labeledImage) {
        try {
            manager.createQuery("UPDATE LabelImage l SET l.score = :score, l.scoredAt = :now, l.labeledImage = :labeledImage" +
                    " WHERE l.id = :imageId")
                    .setParameter("score", score)
                    .setParameter("now", LocalDateTime.now())
                    .setParameter("imageId", imageId)
                    .setParameter("labeledImage", labeledImage)
                    .executeUpdate();
            return true;
        } catch (Exception e) {
            e.printStackTrace();
